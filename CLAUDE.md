...
`effects`, `foundations`, `generics`, `lifetimes`, `memory`, `types`; still
missing (and each still carries its rationale in-spec until written):
`control-flow`, `error-handling`, `functions`, `lexical`, `operators`,
`packages`.
