...
Count the implicit constructors a package needs to write a bool of its own and have it work everywhere a condition is accepted. One to `core`'s `Bool`, because `if` and `elif` are `core` verbs and their conditions are ordinary arguments. And one more to `@primitives$Bool`, because `guard` was grammar and grammar may not name a package's type. Two conversions, to two different destinations, for one idea — and the second existed for no reason a user of the language could be told, only for a reason about where the construct was implemented. We had moved the dependency out of `guard` without noticing that we had moved it onto everyone who wanted to be a condition.

[The previous chapter](#the-exit-that-could-not-become-a-call) had looked at the obvious escape — a bare, condition-less exit, with the condition supplied by whatever branch it sits in — and rejected it on a claim about depth: such an exit "lands one level shallower than a `guard` written at the same depth, because the exit sees the `if` call as its own enclosing call." The observation was right and the conclusion was backwards. Landing one level out is not the defect. It is the whole mechanism, if you point it at the right level.
...
Two rules had to move with it. The exit carries no value, so the invocation it ends has to return `Unit` — and because the caller is decided at the call site, that check belongs to the call rather than to the exiting verb's declaration; the same `guard` is fine in one caller and rejected in the next. And the reach is exactly one level: a verb that calls a verb that exits is not itself exited, or the primitive would be an unwinding mechanism, which is the thing the error model exists to avoid.

It is worth being exact about what this is still not, because the earlier refusal was right about the thing it refused. We would not add a `break`, because a break has to name a scope further up and unwind to it. Nothing here names a scope. The cost is that [Zane still has no break](https://github.com/zane-lang/spec/blob/e1745e4d479fd171aca9040f88061c60e0d1f0f1/spec/control-flow.md#35-there-is-no-unbounded-repetition): nothing ends a repetition and resumes after it, because a `guard` inside a loop body ends the verb the loop was written in, not the loop. A search that has an answer to carry out `return`s it, which crosses blocks the same way; work that must follow a repetition goes after a call to the verb that performs it. A language with no `while` had already accepted the shape of that trade.
...
`guard` reads the same as it always did. It is [no longer a keyword](https://github.com/zane-lang/spec/blob/e1745e4d479fd171aca9040f88061c60e0d1f0f1/spec/syntax.md#72-control-flow-keywords), and the keyword list is now empty.
