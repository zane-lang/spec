...
2. **Generic match.** If the direct phase finds no viable candidate, the called declaration's type parameters are inferred from the static types of the call arguments: a type parameter from an argument's type, a number parameter from the number part of an argument's type. A call never carries a `<>` type-argument list; a type or number may instead be passed as an ordinary argument to a `type` or `number` parameter (see [`generics.md`](generics.md) §5). Inference proceeds under the rules of [`generics.md`](generics.md) §3 and §5, still with no implicit constructor insertions. If exactly one candidate is viable, it is selected. If more than one candidate is viable, the call is an ambiguity error.
...
| One pass handles types and numbers | Both type parameters and number parameters of the called declaration are inferred from the argument types in the same pass, since the two kinds share one slot system (see [`generics.md`](generics.md) §3). |
