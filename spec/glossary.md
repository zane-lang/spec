...
### 3.39 boxed member
- **Meaning:** A **member** — a `struct`/`#struct` field or a `variant`/`#variant` case payload alike — stored as a fixed-size handle inline, with the instance it names placed in the scope's dynamic region: the same two-part representation a `List` uses for its backing store. Boxing is **required** when the member's declared type can lead back to the enclosing type, since no finite inline layout exists for it, and **permitted** otherwise, chosen per type by the implementation. Two different types answer two different questions about it. What the payload **is** follows the member's own declared type: a reference-typed payload keeps its backpointer, identity, and guestability (§3.36), while a value-typed payload — which a reference type may box off a cycle — stays a plain value with none of those. What **becomes of** the payload when the enclosing instance moves, is copied, or dies follows the enclosing type's kind: a reference type *hosts* what it boxes, so the parent destroys the child and rehosting relocates the whole boxed subtree, while a value type *owns* what it boxes and copies it deeply (§3.40). No `&` and no guest source is involved either way. Nothing marks it in the source, and placement is unobservable (§3.25).
...
- **Meaning:** Copying an existing value copies the whole value — its inline bytes, plus a fresh allocation and recursive copy of the payload behind every boxed member (§3.39) it owns — so an original and its copy share no storage. A fresh non-place expression instead constructs directly in its eventual destination, recursively through nested fresh results, and therefore does not pay an extra copy merely because it is bound. For a copied value with no boxed member, which is the ordinary case, the operation is exactly an inline byte copy. Depth is what keeps a copied value transitively alias-free once it may own out-of-line storage, and therefore what lets a value type recurse (§3.2) and stay legal as a concurrent subject (§2.4). The cost is that copying an existing boxed value allocates and is proportional to its structure.
