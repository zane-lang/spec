...
The `#` modifier (§2.1) is the other axis: `struct`/`#struct` are the product pair, `variant`/`#variant` the sum pair. A value mould — `struct` or `variant` — declares a value type: transitively value, so it **MUST NOT** contain a reference-type field, an `&` field, or recurse (§2.2, [`memory.md`](memory.md) §2.10). A reference mould — `#struct` or `#variant` — declares a reference type, which may hold reference-type and `&` fields and may recurse, the compiler breaking a containment cycle with indirection where one would otherwise have no finite size ([`memory.md`](memory.md) §3.3). The body syntax is symmetric across these four combinations; the keyword picks product versus sum and the `#` picks value versus reference. Because `#` marks only a mould, a reference type comes into being only through such a declaration and is always named there (§5.3).
...
| Reference type (`#`) | Single hosting and stable identity; may hold reference-type and `&` fields; may recurse; never relocated once constructed; placement is unobservable |
