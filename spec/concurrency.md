...
| Snapshot of a boxed value | A value owning boxed members is snapshotted by a walk that must validate each handle's complete payload span — offset, size, and alignment, inside a live region — and stop at a depth bound; it allocates from the scope's size stacks, and a retry costs O(structure) |
