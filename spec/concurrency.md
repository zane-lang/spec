...
| Snapshot of a boxed value | A value owning boxed members is snapshotted by a bounded walk that validates structure-directing metadata before typed interpretation and validates each handle's complete payload span — offset, size, and alignment, inside a live region; the copy allocates in its destination binding's scope, rejected attempts return all provisional blocks before retrying, and a retry costs O(structure) |
