...
| Mutation | A spawned mutating call requires a value-typed subject, the only path by which a call writes caller-visible state; at most one mutable borrow per storage location, so a spawn in a loop body must take its subject from storage declared in that body; concurrent reads take a coherent snapshot |
