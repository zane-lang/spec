...
> **Story:** [`stories/generics.md`](../stories/generics.md) tells the story behind these rules — the parameter model, the `<>`/`()` split, the literal-wrapping cost, and the deferred features.
...
- **`Parameters are concept-typed`.** A parameter is declared `name Type` (a type parameter) or `name @concepts$Int` (a number parameter). `Type` and `@concepts$Int` are compiler concept types; the parameter's casing follows what it names — `T` is a type, `n` is a number.
...
A header may mix type parameters and number parameters. A type parameter is declared with the `Type` concept; a number parameter with `@concepts$Int`, the concept type an integer literal carries ([`lexical.md`](lexical.md) §7).
...
`T` ranges over types; `n` ranges over compile-time integers. The argument `64` is an integer literal, so it fills an `@concepts$Int` slot the way any argument fills a slot of its own type. Inside the body, `n` may also be read as a number value — for example, to compute a length — exactly as `data Array<T, n>` uses it to fix the storage size.
...
`Wrapper` is concrete: every field type is fully applied, so `Wrapper`'s layout is known. A named type is always declared with `type Name = struct { ... }` or `type Name = #struct { ... }` (see [`types.md`](types.md) §2); the `struct { ... }` and `#struct { ... }` bodies are type expressions and do not name a type on their own.
...
type Buffer<T Type, n @concepts$Int> = struct { ... }   // header: a type then a number
```

A **verb** — a function, method, or constructor — has no header. It *introduces* each type or number parameter inline, at the parameter's first **marked** occurrence — the first place the name carries its concept (`Type` / `@concepts$Int`):

```zane
Vector<T>(x T Type, y T Type) { ... }             // T introduced on a value parameter
T head(arr Array<T Type, n @concepts$Int>) { ... }       // T, n introduced inside a nested type
Int size(this Buffer<T Type, n @concepts$Int>) { ... }   // T, n introduced on the subject
```
...
```zane
Int size(this Buffer<T Type, n @concepts$Int>) {
    return n;
}
```
...
Arguments fill the header's parameters left to right. A type argument fills a `Type` slot; a number argument fills an `@concepts$Int` slot.
...
A type or number parameter reaches a callable in one of two ways: inferred (introduced inline on a parameter's type or in a nested type) or passed explicitly (declared as a `Type`/`@concepts$Int` value parameter).
...
### 5.3 Explicit parameters (`Type` / `@concepts$Int` value parameters)

A type or number can instead be passed as an ordinary argument by declaring a value parameter of concept type `Type` or `@concepts$Int`. The argument is then written positionally in `()`, like any other value.
...
A `Type` value parameter is usable as a type inside the body (for example, `T(0)`); an `@concepts$Int` value parameter is usable as a number. This is the practical payoff of types being compile-time values: a type handed to a constructor is just an argument the body can execute.
...
A bare source literal carries a compiler concept type, not a concrete storage type. Whether it may drive inference turns on whether that concept type fixes a concrete type. An `@concepts$Int`, `@concepts$Float`, or `@concepts$String` does not — an integer literal may become an `Int` or any other type whose package declares a conversion from `@concepts$Int`, and the compiler does not choose — so such a literal **MUST NOT** drive inference of a type parameter. Wrap it in its destination type:
...
A verb may take that cost on itself instead, by declaring a parameter of the literal's own concept type and doing the wrapping in its body. `@concepts$Int` and `@concepts$Float` are distinct types, so a verb may overload on them ([`functions.md`](functions.md) §4.3), and a call with bare literals selects its overload in the direct phase of resolution ([`functions.md`](functions.md) §5):

```zane
Vector<Int> vec(x @concepts$Int, y @concepts$Int) => Vector(Int(x), Int(y));
Vector<Float> vec(x @concepts$Float, y @concepts$Float) => Vector(Float(x), Float(y));

a Vector<Int> = vec(2, 3);         // legal: selects the @concepts$Int overload
b Vector<Float> = vec(2.5, 3.5);   // legal: selects the @concepts$Float overload
c Vector<Float> = vec(2, 3.5);     // ILLEGAL: no overload takes an integer and then a float
```
...
```zane
Array<T, n>(values @concepts$Array<T Type, n @concepts$Int>) {
...
| Number parameter | Declared `name @concepts$Int` with a lowercase name (`n`); ranges over compile-time integers and resolves to a number value in body positions |
| `Type` / `@concepts$Int` | Compiler concept types; legal only in parameter positions, never as storage; the only two kinds a type's `<>` header holds |
