...
- **`References are bare`.** Inside a body or a nested type, a parameter is referenced by its bare name (`T`, `n`). There is no sigil marking a name as a parameter: a name is *introduced* once — by a type's header or by a verb's first inline occurrence carrying its concept — and the casing rule keeps the two kinds distinct. The mode markers are not exceptions: `&` marks guest storage or passing ([`memory.md`](memory.md) §2.4) and `'` marks borrow passing in a parameter position only ([`memory.md`](memory.md) §2.9). Each marks how a *type* is stored or handed over, and applies to a parameter reference and a concrete type name alike.
