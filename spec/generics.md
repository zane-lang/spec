...
- **`References are bare`.** Inside a body or a nested type, a parameter is referenced by its bare name (`T`, `n`). There is no sigil marking a name as a parameter: a name is *introduced* once — by a type's header or by a verb's first inline occurrence carrying its concept — and the casing rule keeps the two kinds distinct. The storage markers `&T` and `'T` ([`memory.md`](memory.md) §2.4, §2.9) are not exceptions: they mark how a *type* is stored or passed, and apply to a parameter reference and a concrete type name alike.
