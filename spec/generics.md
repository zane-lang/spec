...
A bare source literal carries a compiler concept type, not a concrete storage type. Whether it may drive inference turns on whether that concept type fixes a concrete type. An `@concepts$Integer`, `@concepts$Decimal`, or `@concepts$Text` does not — an integer literal may become an `Int` or any other type whose package declares a conversion from `@concepts$Integer`, and the compiler does not choose — so such a literal **MUST NOT** drive inference of a type parameter. Wrap it in its destination type:
