...
- **`#` means never relocated.** A hosted instance stays where it was constructed for its whole life, so a move transfers hosting without moving bytes and a guest stays valid across it. See [`memory.md`](memory.md) §3.7.
