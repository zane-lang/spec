...
- **`Identity is opt-in`.** A type is a value unless it is marked `#`; the mark makes it a reference type with identity and aliasing, moved rather than copied. Value versus reference is one axis, orthogonal to the product/sum shape of the body.
...
| Identity is opt-in | A type is a value unless marked `#`; `#` adds identity and `&`-aliasing and makes the type moved rather than copied; a value type is transitively value, and either kind may recurse through a boxed member | [`memory.md`](memory.md) §2, [`types.md`](types.md) §2, [`adt.md`](adt.md) §2–§3 |
