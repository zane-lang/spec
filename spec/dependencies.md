...
The URL and version are mangled into safe path components using Go-style path mangling: the URL **scheme** (`https://`, `git@`, and the like) is stripped first, and each `/` in the remaining URL produces a new subdirectory level, so `https://github.com/zane-lang/math` becomes `github.com/zane-lang/math` as nested directories. This path-safety check applies to the URL *after* its scheme has been stripped: if the post-scheme URL or the version tag contains any character that is not safe to use directly as a path component — such as `:`, `@`, `%`, `?`, `#`, or any other character that would be illegal or ambiguous on the host filesystem — `zane add` **MUST** fail immediately with an error rather than attempting to mangle or escape the offending character. (The `:` and `/` of the stripped scheme are therefore exempt by construction; the check screens only the host-and-path remainder that actually becomes directory names.) (`%` is additionally reserved as the symbol separator of §6.1, so forbidding it in tags keeps the version/key boundary unambiguous.)
