...
Binding a plain (swallowed) parameter into `&` storage is illegal, because a swallowed value is owned at the call site while an `&` field lives with the object that holds it — which may outlive the call, leaving the `&` dangling:
