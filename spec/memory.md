...
Only the outer host-capable slot enters guest state after a move; its contained hosts are reached through it. Anchor bookkeeping is therefore O(1) for each contained host of the moved type and never enumerates guests; physical rehosting remains proportional to the bytes, elements, and contained hosts relocated.
