...
| Boxed hosting member | A hosting member whose type can lead back to the enclosing type is stored as a fixed-size handle inline with its enclosing host — a fixed-size scope slot, or another dynamic payload — while the instance the handle names lives in the dynamic region; the compiler chooses this because no finite inline layout exists, and nothing marks it in the source |
