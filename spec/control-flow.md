...
A control-flow intrinsic is called like a function, so its arguments are coercion sites ([`types.md`](types.md) §4.2). A caller therefore writes ordinary values and never unwraps anything: a `Bool` reaches `branch` through the implicit constructor `core` declares to `@primitives$Bool`, and an integer literal reaches `repeat` through `@primitives$Int`'s own implicit constructor ([`types.md`](types.md) §2.7). A type that declares its own conversion is accepted on the same terms.
...
| Control-flow intrinsics | `@controlflow$branch`, `@controlflow$repeat`, and `@controlflow$exitFromCall`; the first two stated over `@primitives$Bool` and `@primitives$Int`, the third over nothing; reachable from any package, with ordinary values reaching them through implicit constructors — those a type's package declares, and the primitive's own from a literal |
