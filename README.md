...
| [`stories/types.md`](stories/types.md) | [`spec/types.md`](spec/types.md) — the two-axis value/reference model, why `#` marks a distinct type rather than a box, constructing a type by what it is, confining `#` to the body forms, why every user-defined type must be named, dropping the `tuple` mould once a named `struct` does the job better, adding named constructors, naming the `enum`'s shape the peer mould, making `Unit` a real fundamental unit type, and the concept-typed lowering exception that grew from one legal line into container-wide coercion before it was taken back out, and the literal that had no way into storage — the storage primitives that construct from their literal's concept, the literal concepts renamed to what other languages call their literals, and the string view beneath `String` |
