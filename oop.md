...
Constructors use the same block-bodied or expression-bodied surface forms as other package-scope functions, except that the return type is implicit and the body constructs the result with `init{ ... }`.
...
| Constructor | Package-scope function declaration named after the type; no `this`; may use block or `=> init{...}` form |
