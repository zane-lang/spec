...
static void zm_reset(void) { zm.top = ZM_TOP0; }
...
static void *zm_alloc_lazy(size_t obj_size) {
    return zm_alloc(obj_size);
}

static inline ZRef zm_create_ref(void *obj, size_t obj_size) {
    (void)obj_size;
    return zm_seg(obj);
}
...
static void test6(void) {
    section("Test 6 -- Guest access via segmented offset vs direct pointer  [100k accesses]");
    double T[RUNS];
    ...
    for(int i=0;i<N;i++){
        objs[i] = (Entity*)zm_alloc_lazy(sizeof(Entity));
        objs[i]->hp = i%100+1;
        refs[i] = zm_create_ref(objs[i], sizeof(Entity));
        direct[i] = objs[i];
    }
    ...
    print_result("Segmented offset (chunk dir cached)", T);
    ...
    print_result("Segmented offset (chunk dir reloaded)", T);

    for(int i=0;i<N;i++) zm_free_lazy(objs[i], sizeof(Entity));
...
static void *zm_alloc_e(size_t s){return zm_alloc_lazy(s);}
static void  zm_free_e (void*p,size_t s){zm_free_lazy(p,s);}
...
static void *zm_af(size_t s){return zm_alloc_lazy(s);}
...
static void test10(void) {
    ...
    double T[RUNS];
    size_t znode_size = sizeof(TNode);

    for(int r=0;r<RUNS;r++){zm_reset();rng_state=0xbadf00dULL+(uint64_t)r;int rem=TREE_NODES;TNode*root=build_tree(&rem,zm_af);assert(rem==0);double t0=now_ns();destroy_zane_norefs(root);T[r]=now_ns()-t0;sink^=(int64_t)rem;}
    ...
    for(int r=0;r<RUNS;r++){
        ...
        int rem=TREE_NODES; TNode*root=build_tree(&rem,zm_af); assert(rem==0);
    ...
    for(int r=0;r<RUNS;r++){rng_state=0xbadf00dULL+(uint64_t)r;int rem=TREE_NODES;TNode*root=build_tree(&rem,ma_af);assert(rem==0);double t0=now_ns();destroy_malloc(root);T[r]=now_ns()-t0;sink^=(int64_t)rem;}
    ...
    for(int r=0;r<RUNS;r++){rng_state=0xbadf00dULL+(uint64_t)r;int rem=TREE_NODES;TNode*root=build_tree(&rem,po_af);assert(rem==0);double t0=now_ns();destroy_pool(root);T[r]=now_ns()-t0;sink^=(int64_t)rem;}
